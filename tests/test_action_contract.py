...
def test_validate_native_action_contract_accepts_declared_shape() -> None:
    manifest = _native_manifest("fastwam-libero", "fastwam")
    chunk = ActionChunk(actions=[[0.0] * 7 for _ in range(32)])

    check = validate_native_action_contract(manifest, chunk, expected_horizon=32)
...
def test_validate_native_action_contract_rejects_bad_shape() -> None:
    manifest = _native_manifest("fastwam-libero", "fastwam")
    chunk = ActionChunk(actions=[[0.0] * 6 for _ in range(31)])

    with pytest.raises(ActionContractError, match="action horizon mismatch"):
        validate_native_action_contract(manifest, chunk, expected_horizon=32)


def test_validate_native_action_contract_rejects_non_finite_values() -> None:
    manifest = _native_manifest("fastwam-libero", "fastwam")
...
def test_maybe_validate_native_action_contract_skips_non_native_modes() -> None:
